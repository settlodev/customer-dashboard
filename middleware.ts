...
//     // If we're on a page that doesn't require immediate auth token, allow it
...
//     // For other pages, redirect to a safe page that can handle missing token
...
export default auth(async (req: NextRequest) => {
  ...
  if (isLoggedIn && !authToken) {
    ...
    if (isCurrentPathSafe) {
      // Allow access but prevent caching
      const response = NextResponse.next();
      response.headers.set('Cache-Control', 'no-store, max-age=0');
      return response;
    } else {
      // Only redirect if we're not already on a safe path
