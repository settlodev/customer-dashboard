...
export default function RecipeForm({ item, onFormSubmitted }: RecipeFormProps) {
  ...
  return (
    <Form {...form}>
      <FormError message={response?.message} />
      <form className="space-y-6" onSubmit={form.handleSubmit(submitData)}>
        <CardContent className="space-y-6 max-w-screen-md px-4 sm:px-6">
          {/* Recipe Name */}
          <FormField
            control={form.control}
            name="name"
          ...
          <div className="space-y-4">
            ...
            {fields.map((field, index) => (
              <div
                key={field.id}
                className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-start p-3 sm:p-4 rounded-lg border bg-card"
              >
                ...
                <Button
                  ...
                  onClick={() => remove(index)}
                  disabled={fields.length === 1 || isPending}
                  className="self-center sm:self-start shrink-0"
                >
                  <Trash2 className="w-4 h-4 text-red-500" />
                </Button>
              </div>
        ...
        <div className="flex h-5 items-center space-x-4 mt-4 px-4 sm:px-6">
          <CancelButton />
          <Separator orientation="vertical" />
          <SubmitButton
            isPending={isPending}
