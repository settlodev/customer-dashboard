...
import {Building2, Loader2Icon, Mail, MapPin, Phone, User, Clock, Store} from "lucide-react";
import { useRouter } from "next/navigation";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { PhoneInput } from "@/components/ui/phone-input";
import CancelButton from "@/components/widgets/cancel-button";
import SubmitButton from "@/components/widgets/submit-button";
import { createWarehouse, updateWarehouse } from "@/lib/actions/warehouse/register-action";
...
const timeToBusinessTimeName = (timeString: string): string => {
  ...
  const businessTime = businessTimes.find(bt => {
    // Assuming businessTime.name is in format like "07:00" or similar
    return bt.name === timePart || bt.name === timeString;
  });
  
  return businessTime?.name || timeString;
};

function WarehouseForm({ item }: { item: Warehouses | null | undefined }) {
    console.log("The warehouse item is", item);
    ...
    useEffect(() => {
        if (!isLoadingBusiness) {
            const formValues = {
                ...
                openingTime: item?.openingTime ? timeToBusinessTimeName(item.openingTime) : "",
                closingTime: item?.closingTime ? timeToBusinessTimeName(item.closingTime) : "",
            };
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <CardTitle className="text-2xl font-bold">
                                {item ? "Update Warehouse" : "Create New Warehouse"}
                            </CardTitle>
                            <p className="text-blue-100 mt-1">
                                {item ? "Modify warehouse details" : "Add a new warehouse to your business"}
                            </p>
                        </div>
                    </div>
                </CardHeader> */}
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid md:grid-cols-2 gap-4">
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                <FormLabel className="text-gray-700 font-medium">Opening Time</FormLabel>
                                                <FormControl>
                                                    <Select
                                                        disabled={isPending}
                                                    ...
                                                    >
                                                        ...
                                                        <SelectContent>
                                                            {businessTimes.length > 0
                                                                ? businessTimes.map((time: BusinessTimeType, index: number) => (
                                                                    <SelectItem key={index} value={time.name}>
                                                                        {time.label}
                                                                    </SelectItem>
                                                                ))
                                                                : <SelectItem value="no-times">No times available</SelectItem>
                                                            }
                                                        </SelectContent>
                                                    </Select>
                                                </FormControl>
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                <FormLabel className="text-gray-700 font-medium">Closing Time</FormLabel>
                                                <FormControl>
                                                    <Select
                                                        disabled={isPending}
                                                        onValueChange={field.onChange}
                            ...
                            <div className="flex flex-col sm:flex-row items-center gap-4 pt-8 border-t">
                                ...
                                {isPending ? (
                                    ...
                                    <SubmitButton
                                        ...
                                        className="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-8 py-3 rounded-lg font-medium transition-all transform hover:scale-105 min-w-[160px]"
                                    />
                                )}
                            </div>
                        </form>
