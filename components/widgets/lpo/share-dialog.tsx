...
import { revokeLpoShare, shareLpo } from "@/lib/actions/lpo-actions";
...
export function LpoShareButton({ lpo }: { lpo: Lpo }) {
  const [open, setOpen] = useState(false);
  const [isMinting, startMint] = useTransition();
  const [isRevoking, startRevoke] = useTransition();
  const [token, setToken] = useState<string | null>(lpo.shareToken);
  ...
  const mintToken = () => {
    startMint(async () => {
      const result = await shareLpo(lpo.id);
      if ("error" in result) {
        toast({
          variant: "destructive",
  ...
  const handleRevoke = () => {
    if (!token) return;
    startRevoke(async () => {
      const result = await revokeLpoShare(lpo.id);
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription asChild>
              <div className="space-y-2">
                <p>Anyone with this link can view {lpo.lpoNumber}.</p>
                <p>
                  The supplier can accept or reject the order on the public
                  page. Status reflects the latest state.
                </p>
              </div>
            </DialogDescription>
          </DialogHeader>
          ...
          {isMinting || !token ? (
            ...
            <div className="space-y-3">
              ...
              {lpo.shareTokenIssuedAt && (
                <p className="text-xs text-muted-foreground">
                  Link first issued{" "}
                  {new Date(lpo.shareTokenIssuedAt).toLocaleString("en-GB", {
                    day: "2-digit",
                    month: "short",
                    year: "numeric",
                    hour: "2-digit",
