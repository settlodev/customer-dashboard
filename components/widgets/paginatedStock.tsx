...
const PaginatedStockTable = ({ movements, itemsPerPage = 10 }: { movements: StockMovement[]; itemsPerPage?: number }) => {
  ...
  }, [sortedMovements, startDate, endDate, filterActive]);
    ...
    <div className='w-full lg:w-auto'>
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:flex items-center gap-2'>
        ...
        <div className="flex gap-2 w-full lg:w-auto lg:ml-4 mt-2 lg:mt-4">
          <Button variant="default" className="flex-1" onClick={handleFilter} disabled={isFiltering}>
            {isFiltering ? "Filtering..." : "Filter"} 
          </Button>
          {filterActive && (
            <Button variant="outline" className="flex-1" onClick={resetFilter} disabled={isFiltering}>
              Reset
            </Button>
          )}
        </div>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-gray-50 border-b">
            ...
            <th className="text-left p-2 font-medium hidden md:table-cell">Staff</th>
          ...
          {currentItems.length > 0 ? (
            currentItems.map((movement, index) => (
              ...
              >
                ...
                <td className="p-2">
                  <span className={`inline-flex items-center px-3 py-2 rounded-md text-xs font-medium ${
                ...
                <td className="p-2 md:hidden">
                  <div className="text-xs text-gray-500">
                    ...
                    <div>{Intl.DateTimeFormat(undefined, {
                      month: 'short',
                      day: 'numeric',
                      hour: '2-digit',
                      minute: '2-digit',
                      hour12: false
                    }).format(new Date(movement.dateCreated))}</div>
                  </div>
                </td>
              </tr>
            ))
    ...
    {filteredMovements.length > 0 && (
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mt-4 gap-4">
        <div className="text-sm text-gray-500 order-2 sm:order-1">
          Showing {startIndex + 1} to {Math.min(endIndex, filteredMovements.length)} of {filteredMovements.length} entries
        </div>
