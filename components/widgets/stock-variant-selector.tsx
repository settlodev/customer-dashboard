...
const StockVariantSelector: React.FC<Props> = ({
  ...
  // NEW: track which ids we've already fetched, so we don't refetch on every
  ...
  // FIX 2: prefer selectedVariantInfo (which is set synchronously when a match
  // is found in stocks) over the lookup, so we never flash placeholder.
  const selectedOption = useMemo(() => {
    if (!value) return null;
    if (selectedVariantInfo && selectedVariantInfo.id === value) {
      return selectedVariantInfo;
  ...
  const handleSelect = useCallback(
    (option: { id: string; displayName: string }) => {
      ...
      if (newValue) {
        ...
        // mark as fetched so the effect doesn't refetch
  ...
  // FIX 3: only show "Loading..." if we're loading AND we don't already have
  // a displayable selection. This prevents the placeholder flash.
  const displayText = useMemo(() => {
    if (selectedOption) return selectedOption.displayName;
