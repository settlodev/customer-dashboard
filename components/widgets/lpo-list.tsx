...
// components/widgets/lpo-list.tsx
"use client";
...
interface LPOSelectionListProps {
  lpos: StockPurchase[];
  onSelect: (lpo: StockPurchase) => void;
}

export function LPOSelectionList({ lpos, onSelect }: LPOSelectionListProps) {
  const [searchQuery, setSearchQuery] = useState("");

  const filtered = lpos.filter((lpo) => {
    const q = searchQuery.toLowerCase();
    return (
      lpo.orderNumber.toLowerCase().includes(q) ||
      lpo.supplierName.toLowerCase().includes(q) ||
      lpo.stockIntakePurchaseOrderItems.some((item) =>
        item.stockVariantName?.toLowerCase().includes(q),
      )
    );
  });
  ...
  return (
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map((lpo) => {
            const totalQty = lpo.stockIntakePurchaseOrderItems.reduce(
              (s, i) => s + i.quantity,
              0,
            );
            ...
            return (
              <div
                key={lpo.id}
                className="flex flex-col sm:flex-row sm:items-center gap-4 rounded-xl border border-gray-200 bg-white px-4 py-3 hover:border-orange-200 hover:bg-orange-50/30 transition-colors group"
              >
                {/* Left: PO info */}
                <div className="flex-1 min-w-0 space-y-2">
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
                    ...
                    <span className="flex items-center gap-1.5">
                      ...
                      Delivery:{" "}
                      {format(new Date(lpo.deliveryDate), "MMM dd, yyyy")}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 items-center">
                    <span className="flex items-center gap-1 text-xs text-muted-foreground">
                      ...
                      {lpo.stockIntakePurchaseOrderItems.length} item(s) ·
                    </span>
                    <span className="flex items-center gap-1 text-xs text-muted-foreground">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {lpo.stockIntakePurchaseOrderItems
                      .slice(0, 4)
                      .map((item) => (
                        <span
                          key={item.id}
                    ...
                    {lpo.stockIntakePurchaseOrderItems.length > 4 && (
                      <span className="text-xs text-muted-foreground px-1 py-0.5">
                        +{lpo.stockIntakePurchaseOrderItems.length - 4} more
                      </span>
                    )}
                  </div>
                </div>
                ...
                <Button
                  ...
                  className="shrink-0 group-hover:border-orange-300 group-hover:text-orange-600 group-hover:bg-orange-50 transition-colors"
                >
                  Select
