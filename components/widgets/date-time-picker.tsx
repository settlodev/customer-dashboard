...
type handleTimeChangeType = "hour" | "minutes";
type FieldType = {
  value: Date | undefined; // Changed from string | undefined to Date | undefined
  onChange: (value: Date) => void; // Changed from string to Date
};

const DateTimePickerTwo = ({
  field,
  date,
  setDate,
  handleTimeChange,
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          {field.value
            ? format(field.value, "MM/dd/yyyy HH:mm") // Use field.value directly (Date object)
            : format(new Date(), "MM/dd/yyyy HH:mm")}
          <CalendarIcon className="ml-auto h-4 w-4 opacity-50" />
        </Button>
      </PopoverTrigger>
            ...
            <Calendar
              ...
              onSelect={(selectedDate) => {
                if (selectedDate) {
                  setDate(selectedDate);
                  onDateSelect(selectedDate);
                  field.onChange(selectedDate); // Pass Date object directly
                }
              }}
              disabled={(date) => {
                if (minDate && date < minDate) return true;
                  ...
                  {Array.from({ length: 24 }, (_, i) => i)
                    ...
                    .map((hour) => (
                      <Button
                        ...
                        size="icon"
                        variant={
                          field.value && field.value.getHours() === hour // Use field.value directly
                            ? "default"
                            : "ghost"
                        }
                        className={cn(
                        ...
                        onClick={() => {
                          ...
                          const newDate = new Date(date || new Date());
                          newDate.setHours(hour);
                          field.onChange(newDate); // Pass Date object directly
                        }}
                      >
                        {hour}
                      </Button>
                ...
                <div className="flex sm:flex-col p-2">
                  {Array.from({ length: 12 }, (_, i) => i * 5).map((minute) => (
                    <Button
                      ...
                      size="icon"
                      variant={
                        field.value && field.value.getMinutes() === minute // Use field.value directly
                          ? "default"
                          : "ghost"
                      }
                      className={cn(
...
export default DateTimePickerTwo;
