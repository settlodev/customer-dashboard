...
const PricingCard: React.FC<PricingCardProps> = ({
  ...
  // Calculate annual price (12 months)
  const annualAmount = amount * 12;

  // Format monthly price
  const formattedMonthlyAmount = amount.toLocaleString("en-US", {
    style: "currency",
    currency: "TZS",
    minimumFractionDigits: 0,
    maximumFractionDigits: 0,
  ...
  return (
    <div
      className={`relative flex flex-col bg-white rounded-2xl p-6 transition-all duration-300 hover:shadow-xl border 
      ${isPopular ? "border-emerald-200 shadow-lg scale-105" : "border-gray-100"}`}
    >
      {isPopular && (
        <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-emerald-500 text-white px-4 py-1 rounded-full text-sm font-medium flex items-center gap-1">
    ...
    <section id="pricing" className="relative w-full overflow-hidden py-24">
      ...
      <div className="relative container mx-auto px-4">
        <div className="max-w-3xl mx-auto text-center space-y-6 mb-16">
          ...
          <p className="text-xl text-gray-600">
            Our flexible pricing options are designed to grow with your
            business, ensuring you have all the tools you need at every stage.
          </p>
        </div>
