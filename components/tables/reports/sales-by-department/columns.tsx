...
import { ArrowDownRight, ArrowUpDown, ArrowUpRight, Building2 } from "lucide-react";

import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";

/** One row of the sales-by-department summary. Sales are fanned out to every
 * department a product's categories roll up to, so totals across rows can
 * exceed the overall net (a product spanning two departments counts toward
 * both). */
export interface DepartmentSalesRow {
  id: string;
  name: string;
  /** Distinct products in this department that sold in the period. */
  products: number;
  qty: number;
  gross: number;
  net: number;
...
export function buildSalesByDepartmentColumns({
  currency,
}: BuildColumnsOptions): ColumnDef<DepartmentSalesRow>[] {
  return [
    {
      accessorKey: "name",
      enableHiding: false,
