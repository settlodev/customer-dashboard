...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const handleArchive = async () => {
    ...
    try {
      ...
      if (result.responseType === "success") {
        ...
        toast({ variant: "destructive", title: "Error", description: result.message || "Failed to archive category" });
      }
    } catch (error) {
      toast({ variant: "destructive", title: "Error", description: (error as Error).message || "Failed to archive category" });
    } finally {
  ...
  const handleUnarchive = async () => {
    ...
    try {
      ...
      if (result.responseType === "success") {
        ...
        toast({ variant: "destructive", title: "Error", description: result.message || "Failed to restore category" });
      }
    } catch (error) {
      toast({ variant: "destructive", title: "Error", description: (error as Error).message || "Failed to restore category" });
    } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Error", description: (error as Error).message || "Failed to delete category" });
    } finally {
      setIsDeleting(false);
    ...
    <>
      <DropdownMenu modal={false}>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem onClick={() => router.push(`/categories/${data.id}/edit`)}>
