...
import {
  archiveProductCollection,
  unarchiveProductCollection,
...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const handleArchive = async () => {
    setIsArchiving(true);
    try {
      const result = await archiveProductCollection(data.id);
      if (result.responseType === "success") {
        toast({ variant: "success", title: "Archived", description: `${data.name} has been archived.` });
        router.refresh();
      } else {
        toast({ variant: "destructive", title: "Error", description: result.message || "Failed to archive collection" });
      }
    } catch (error) {
      toast({ variant: "destructive", title: "Error", description: (error as Error).message || "Failed to archive collection" });
    } finally {
  ...
  const handleUnarchive = async () => {
    ...
    try {
      const result = await unarchiveProductCollection(data.id);
      if (result.responseType === "success") {
        toast({ variant: "success", title: "Restored", description: `${data.name} has been restored.` });
        router.refresh();
      } else {
        toast({ variant: "destructive", title: "Error", description: result.message || "Failed to restore collection" });
      }
    } catch (error) {
      toast({ variant: "destructive", title: "Error", description: (error as Error).message || "Failed to restore collection" });
    } finally {
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      await deleteProductCollection(data.id);
      toast({ variant: "warning", title: "Deleted", description: `${data.name} has been permanently deleted.` });
      router.refresh();
    } catch (error) {
      toast({ variant: "destructive", title: "Error", description: (error as Error).message || "Failed to delete collection" });
    } finally {
      setIsDeleting(false);
    ...
    <>
      <DropdownMenu modal={false}>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem onClick={() => router.push(`/product-collections/${data.id}`)}>
