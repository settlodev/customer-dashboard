...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const handleArchive = async () => {
    setIsArchiving(true);
    try {
      await archiveAddonGroup(data.id);
      toast({
        variant: "success",
        title: "Archived",
      ...
      toast({
        ...
        title: "Error",
        description:
          (error as Error).message || "Failed to archive addon group",
      });
    } finally {
  ...
  const handleUnarchive = async () => {
    ...
    try {
      await unarchiveAddonGroup(data.id);
      toast({
        variant: "success",
        title: "Restored",
      ...
      toast({
        ...
        title: "Error",
        description:
          (error as Error).message || "Failed to restore addon group",
      });
    } finally {
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      await deleteAddonGroup(data.id);
      toast({
        variant: "warning",
        title: "Deleted",
      ...
      toast({
        ...
        title: "Error",
        description:
          (error as Error).message || "Failed to delete addon group",
      });
    } finally {
      setIsDeleting(false);
    ...
    <>
      <DropdownMenu modal={false}>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem
            onClick={() => router.push(`/addon-groups/${data.id}`)}
          >
            <Eye className="mr-2 h-4 w-4" />
            View
          </DropdownMenuItem>
          <DropdownMenuItem
            onClick={() => router.push(`/addon-groups/${data.id}/edit`)}
          >
