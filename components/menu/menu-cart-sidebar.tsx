...
interface CartViewProps {
  ...
  subtotal: number;
  itemCount: number;
  minAmountMet: boolean;
  maxAmountMet: boolean;
  maxItemsMet: boolean;
  onClose: () => void;
}
...
function CartView({
  ...
  const { state, removeFromCart, updateQuantity } = useMenuCart();
  ...
  return (
    <div className="p-4 space-y-3">
      {state.items.map((item) => {
        ...
        const lineTotal = (item.price + modTotal + addonTotal) * item.quantity;
                ...
                {(item.selectedModifiers.length > 0 ||
                  ...
                  <div className="mt-1 space-y-0.5">
                    {item.selectedModifiers.map((m) => (
                      ...
                      >
                        + {m.itemName}
                        {m.price > 0 && ` (${m.price.toLocaleString()} TZS)`}
                      </p>
                    ))}
                    {item.selectedAddons.map((a) => (
                      <p key={a.addonId} className="text-[11px] text-gray-400">
...
interface DetailsViewProps {
  ...
  setServingType: (type: ServingType) => void;
...
interface ConfirmationViewProps {
  primaryColor: string;
  businessName: string;
  orderResponse: MenuOrderResponse;
  onClose: () => void;
}
...
export function MenuCartSidebar({
  ...
  const { state, clearCart, closeCart, getSubtotal, getItemCount } =
    useMenuCart();
  ...
  const [orderResponse, setOrderResponse] = useState<MenuOrderResponse | null>(
    null,
  );
  ...
  const canCheckout =
    minAmountMet && maxAmountMet && maxItemsMet && itemCount > 0;
  ...
  const handleSubmit = async () => {
    ...
    const order: MenuOrderRequest = {
      ...
      deliveryAddress: servingType === "DELIVERY" ? deliveryAddress : undefined,
        ...
        <div className="flex-1 overflow-y-auto">
          {step === "cart" && (
            <CartView
              ...
              subtotal={subtotal}
              itemCount={itemCount}
              minAmountMet={minAmountMet}
              maxAmountMet={maxAmountMet}
              maxItemsMet={maxItemsMet}
              onClose={handleClose}
            />
          )}
          {step === "details" && (
