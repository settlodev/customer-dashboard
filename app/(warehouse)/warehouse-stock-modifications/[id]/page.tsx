...
export default async function Page({ params }: { params: Params }) {
  ...
  if (isNew) {
    return (
      <div className="flex-1 px-4 pt-4 pb-8 md:px-8 mt-12">
        <BreadcrumbsNav
          items={[
            { title: "Stock Modifications", link: "/warehouse-stock-modifications" },
            { title: "New", link: "" },
          ]}
        />
        <h1 className="text-2xl font-bold mt-4">Stock Modification</h1>
  ...
  const netChange =
    item.items?.reduce((sum, line) => sum + Number(line.quantityChange), 0) ?? 0;

  return (
    <div className="flex-1 space-y-4 px-4 pt-4 pb-8 md:px-8 mt-12">
      <BreadcrumbsNav
        items={[
          { title: "Stock Modifications", link: "/warehouse-stock-modifications" },
      ...
      <Card>
        <CardContent className="pt-6 grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
          ...
          <Meta label="Warehouse" value={item.locationName || "—"} />
          <Meta
            label="Net change"
            value={
              netChange > 0
                  ...
                  {item.items.map((line) => {
                    ...
                    const signed =
                      line.quantityChange > 0
                        ? `+${line.quantityChange.toLocaleString()}`
                        : line.quantityChange.toLocaleString();
                    return (
                      <tr key={line.id} className="hover:bg-gray-50/50">
                        ...
                        <td className="px-4 py-3 text-right text-gray-600">
                          {line.previousQuantity.toLocaleString()}
                        </td>
                        <td
                          className={`px-4 py-3 text-right font-medium ${line.quantityChange > 0 ? "text-green-600" : "text-red-600"}`}
                        >
                          {signed}
                        </td>
                        <td className="px-4 py-3 text-right">
                          {line.newQuantity.toLocaleString()}
                        </td>
                        <td className="px-4 py-3 text-right">
                          {line.unitCost != null ? (
