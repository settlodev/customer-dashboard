...
const breadCrumbItems = [
  { title: "Stock Intake", link: "/warehouse-stock-intakes" },
];
type Params = {
...
async function Page({ searchParams }: Params) {
  ...
  const responseData = await searchStockIntakesFromWarehouse(
    q,
    page,
    pageLimit,
  );
    ...
    <div className="flex-1 space-y-4 md:p-8 pt-6 px-4 mt-12">
      <div className={`flex items-center justify-between mb-2`}>
        ...
        <div className={`flex items-center space-x-2`}>
          <Button>
            <Link href={`/warehouse-stock-intakes/new`}>
              Record Stock Intake
            </Link>
          </Button>
        </div>
      ...
      {total > 0 || q != "" ? (
        ...
        <NoItems
          newItemUrl={`/warehouse-stock-intakes/new`}
          itemName={`Stock intakes`}
        />
      )}
    </div>
  );
