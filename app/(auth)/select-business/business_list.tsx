...
const BusinessList = ({ businesses }: { businesses: Business[] }) => {
  ...
  const handleBusinessSelect = useCallback(
    async (selectedBusiness: Business, index: number) => {
      ...
      try {
        ...
        // Hard navigation across subdomain — can't use next/router for this.
        // The middleware on the destination subdomain will handle further
        // redirects (e.g. to /select-location if no location is selected).
        window.location.href = redirectUrl;
      } catch (error) {
        Sentry.captureException(error);
        setIsRedirecting(false);
        ...
        const message =
          error instanceof Error && error.message.includes("slug")
            ? "This business is not yet configured for the new platform. Please contact support."
  ...
  // Note: handleBusinessSelect is intentionally omitted from deps to avoid
  // re-running on every render. We only want this to fire once on mount.
  useEffect(() => {
    if (businesses.length === 1 && !isLoading && !isRedirecting) {
      handleBusinessSelect(businesses[0], 0);
    }
            ...
            <button
              ...
              className={cn(
                ...
                pendingIndex === index
                  ? "border-primary/30 bg-primary/5"
                  : "border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 hover:border-primary/30 hover:shadow-sm",
                (isLoading || isRedirecting) && "opacity-60 cursor-not-allowed",
              )}
            >
              <div className="w-11 h-11 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0">
                {bus.logo ? (
