...
export function ModifierGroupDetailActions({
  group,
}: {
  group: ModifierGroup;
}) {
  ...
  const handleArchive = async () => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: (error as Error).message || "Failed to archive",
      });
    } finally {
      setLoading(false);
  ...
  const handleUnarchive = async () => {
    ...
    try {
      ...
      toast({
        title: "Restored",
        description: `${group.name} has been restored.`,
      });
      router.refresh();
    } catch (error) {
      toast({
        variant: "destructive",
        title: "Error",
        description: (error as Error).message || "Failed to restore",
      });
    } finally {
      setLoading(false);
    }
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader className="space-y-3">
            ...
            <DialogDescription className="text-center">
              This group and all its options will be archived.
            </DialogDescription>
          </DialogHeader>
          <div className="flex items-start gap-3 rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/30 p-3 my-2">
            ...
            <p className="text-xs text-amber-700 dark:text-amber-400">
              Archived groups stay attached to their products but won&apos;t
              show up in the picker for new attachments. They can be restored
              later.
            </p>
          </div>
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader className="space-y-3">
            ...
            <DialogDescription className="text-center">
              This action is permanent and detaches the group from every
          ...
          <div className="flex items-start gap-3 rounded-lg border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/30 p-3 my-2">
            ...
            <p className="text-xs text-red-700 dark:text-red-400">
              This action cannot be undone. All options will be removed.
            </p>
          </div>
          <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-2 pt-2">
            <Button
