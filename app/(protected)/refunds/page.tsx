...
async function Page({ searchParams }: Params) {
  ...
  const responseData = await softFetch(
    searchOrderItemRefunds(q, page, pageLimit),
  );
  ...
  return (
    <PageShell>
      ...
      <PageHeader title="Items Refunded" subtitle="A list of all orders" />
      <PageBody>
        {!responseData ? (
          ...
          <div className="h-[calc(100vh-240px)] border border-dashed">
            <div className="m-auto flex h-full w-full flex-col items-center justify-center gap-2">
              <h1 className="text-[1.5rem] font-bold leading-tight">
                No item refunded found
              </h1>
              <p className="text-sm text-center text-muted-foreground">
                There are no order records found on item at the moment.
              </p>
            </div>
          </div>
        )}
