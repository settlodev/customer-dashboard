...
const hashLocationId = (locationId: string) => {
  ...
  // Convert to base64-like encoding and remove special chars
  const base64Hash = btoa(String.fromCharCode(...new Uint8Array(new Int32Array([hash]).buffer)))
    .replace(/\+/g, '-')
    .replace(/\//g, '_')
    .replace(/=/g, '');
...
const QRCodeGenerator = () => {
  ...
  // Generate menu URL with hashed location ID
  const getMenuUrl = () => {
    if (typeof window !== 'undefined') {
      const baseUrl = `${window.location.origin}/menu`;
      
      // Add location ID parameter if available, but hash it first
      if (location && location.id) {
        const hashedId = hashLocationId(location.id);
        return `${baseUrl}?lid=${hashedId}`;
      }
