...
const OrderDetailsPage = async ({ params }: { params: Params }) => {
  ...
  const calculateMetrics = () => {
    ...
    const processingTime = orderData.closedDate
      ? new Date(orderData.closedDate).getTime() -
        new Date(orderData.openedDate).getTime()
      : 0;
  ...
  const departmentAnalysis = () => {
    const departments = orderData.items.reduce(
      (acc, item) => {
        const dept = acc[item.departmentName] || {
          revenue: 0,
          profit: 0,
          count: 0,
        };
  ...
  const sortedItems = [...orderData.items].sort(
    (a, b) =>
      b.price * b.quantity +
      b.grossProfit -
      (a.price * a.quantity + a.grossProfit),
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 mt-3">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <BreadcrumbsNav
          items={[
            { title: "Order Details", link: `/orders/${resolvedParams.id}` },
          ]}
        />
        ...
        {metrics.profitMargin < 0 && (
          <Alert className="bg-red-50 text-red-800 border-red-200">
            ...
            <AlertDescription>
              This order has a negative profit margin of{" "}
              {metrics.profitMargin.toFixed(2)}%. Consider reviewing pricing
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <CardHeader className="border-b">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    <Badge
                      variant="outline"
                      className="bg-blue-50 items-center sm:text-sm"
                    >
          ...
          <div className="space-y-6">
            <OrderInformationCard orderData={orderData} metrics={metrics} />

            <OrderFinancialSummaryCard orderData={orderData} />
