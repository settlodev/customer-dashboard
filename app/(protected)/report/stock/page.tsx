...
const StockHistoryDashboard = () => {
  ...
  const downloadLowStockPdf = () => {
    if (!history?.lowStockItems?.length) return;

    const doc = new jsPDF();
    const now = new Date().toLocaleDateString();

    doc.setFontSize(16);
    doc.text("Low Stock Items Report", 14, 18);
    doc.setFontSize(10);
    ...
    autoTable(doc, {
      ...
      body: history.lowStockItems.map((item, index) => [
        index + 1,
      ...
      styles: { fontSize: 10 },
      columnStyles: { 2: { textColor: [217, 119, 6] } },
    });
  ...
  const downloadOutOfStockPdf = () => {
    if (!history?.outOfStockItems?.length) return;

    const doc = new jsPDF();
    const now = new Date().toLocaleDateString();

    doc.setFontSize(16);
    doc.text("Out of Stock Items Report", 14, 18);
    doc.setFontSize(10);
    doc.setTextColor(120);
    doc.text(`Generated: ${now}`, 14, 25);
    doc.setTextColor(0);
    ...
    autoTable(doc, {
      startY: 30,
      head: [["#", "Stock Item"]],
      body: history.outOfStockItems.map((item, index) => [
        index + 1,
        `${item.stockName} - ${item.stockVariantName}`,
      ]),
      ...
      columnStyles: { 0: { textColor: [100, 100, 100] } },
    });
