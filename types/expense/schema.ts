...
export const ExpenseSchema = object({
  name: string({ required_error: "Expense title is required" }).min(
    3,
  ...
  staff: string({ required_error: "Staff member is required" }).uuid(
  ...
  amount: preprocess(
    ...
    number({ message: "Please total expense amount should be valid number" })
      .nonnegative({
        message: "Please total expense amount should be positive number",
      })
      .gt(0, {
        message: "Please total expense amount should be greater than 0",
      }),
  ),
