...
export const LocationSchema = object({
    name: string().min(2, 'Name of location must be less than 3 characters').max(15, 'Name of location can not be more than 15characters'),
    phone: string().min(8, 'Phone number must be less than 10 characters').max(15, 'Phone number can not be more than 15characters'),
