...
//     (val) => {
//       if (typeof val === "string" && val.trim() !== "") {
//         return parseFloat(val); // Use `parseFloat` instead of `parseInt`
//       }
//       if (typeof val === "number") {
//         return val; // Ensure numbers remain numbers
//       }
...
//   stockItem: string().nullable().optional(), // NEW: separate field for stock
//   recipeItem: string().nullable().optional(), // NEW: separate field for recipe
...
// Update your VariantSchema to remove the trackItem validation in superRefine
...
export const VariantSchema = object({
  ...
  trackInventory: boolean().default(false),
  trackingType: string().optional().nullish(),
  trackItem: string().optional().nullish(), // Keep this for backward compatibility
  stockItem: string().nullable().optional(),
  recipeItem: string().nullable().optional(),
});

// Remove the superRefine validation since we'll handle it differently
