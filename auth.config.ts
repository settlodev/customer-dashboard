    ...
    Credentials({
      async authorize(credentials: any) {
        ...
        try {
          ...
          // Capture upstream routing headers
          const upstreamService = response.headers.get("x-upstream-service");
          const handoffToken = response.headers.get("x-handoff-token");

          const userData = await response.json();

          console.log("🔵 [authorize] x-upstream-service:", upstreamService);

          return {
            ...userData,
            id: userData.userId, // Auth.js expects `id` on the user object
            upstreamService: upstreamService || null,
            handoffToken: handoffToken || null,
          };
        } catch (error: any) {
