...
/**
 * Grades each entry of a variant's ledger. Shared by the on-screen ledger and
 * its CSV export so the two never disagree about which rows are flagged.
 *
 ...
 */
export function analyseLedgerRows(movements: StockMovement[]): LedgerRow[] {
  return movements.map((m) => {
    ...
    if (!untracked) {
      ...
      if (Math.abs(delta) > LEDGER_EPS) mathDelta = delta;
    }
    ...
    if (prev != null && m.previousClosingBalance != null) {
      ...
      if (Math.abs(gap) > LEDGER_EPS) chainGap = gap;
    }
