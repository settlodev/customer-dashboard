...
export const getSubscriptionAddons = async (
  ...
  try {
    ...
    const data = await apiClient.post(`/api/subscription-addons`, query);
...
export const validateDiscountCode = async (
  ...
  try {
    ...
    const response = await apiClient.post(
      `/api/subscription-discounts/validate-discount-code`,
      payload,
    );
    return parseStringify(response);
  } catch (error: any) {
...
export const paySubscription = async (
  subscription: z.infer<typeof RenewSubscriptionSchema>,
) => {
  let formResponse: FormResponse | null = null;
  ...
  const location = (await getCurrentLocation()) || {
    id: validSubscription.data.locationId,
  };
  ...
  try {
    const apiClient = new ApiClient();
    const response: SubscriptionResponse = await apiClient.post(
      `/api/subscription-payments/${location?.id}/subscribe`,
      payload,
    );
...
export const verifyPayment = async (
  ...
  try {
    const apiClient = new ApiClient();
    const response = await apiClient.get(
      `/api/invoice-payments/${invoice}/${transactionId}`,
    );
