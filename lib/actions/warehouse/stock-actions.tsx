...
export const getStockFromWarehouse = async (
  id: UUID,
): Promise<ApiResponse<Stock>> => {
  const apiClient = new ApiClient();
  const query = {
...
export const deleteStockFromWarehouse = async (
  id: UUID,
): Promise<FormResponse | void> => {
  ...
  try {
    ...
    await apiClient.delete(`/api/warehouse-stock/${warehouse?.id}/${id}`);
    revalidatePath("/warehouse-stock-variants");
  } catch (error: any) {
    const formattedError = await error;
...
export const uploadStockCSVForWarehouse = async ({
  fileData,
  fileName,
}: {
  fileData: string;
...
export const stockReportFromWarehouse =
  async (): Promise<StockHistory | null> => {
    await getAuthenticatedUser();
