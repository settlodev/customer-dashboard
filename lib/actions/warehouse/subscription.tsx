...
export const searchWarehousesSubscriptionPackages = async (): Promise<
  ApiResponse<any>
> => {
  const q = "";
  const page = 0;
...
export const payWarehouseInvoice = async (
  id: UUID,
  email: string,
  phone: string,
) => {
  ...
  try {
    ...
    const response = await apiClient.post(
      `/api/warehouse-invoice-payments/${id}/create`,
      payload,
    );
...
export const verifyWarehousePayment = async (
  ...
  try {
    ...
    const response = await apiClient.get(
      `/api/warehouse-invoice-payments/${invoice}/${transactionId}`,
    );
    return parseStringify(response);
  } catch (error) {
    throw error;
