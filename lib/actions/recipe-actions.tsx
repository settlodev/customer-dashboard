...
export const fetchRecipes = async (): Promise<Recipe[]> => {
  ...
  try {
    ...
    const addonData = await apiClient.get(`/api/recipes/${location?.id}`);
...
export const createRecipe = async (
  ...
  const payload = {
    ...
    recipeStockVariants:
      validRecipeData.data.stockVariants?.map((variant) => ({
  ...
  try {
    ...
    await apiClient.post(`/api/recipes/${location?.id}/create`, payload);
...
export const updateRecipe = async (
  ...
  try {
    ...
    payload.recipeStockVariants.forEach((newVariant) => {
      if (
        newVariant.stockVariant &&
        existingVariantsMap.has(newVariant.stockVariant)
      ) {
    ...
    await apiClient.put(`/api/recipes/${location?.id}/${id}`, finalPayload);
...
export const deleteRecipe = async (id: UUID): Promise<void> => {
  ...
  try {
    ...
    await apiClient.delete(`/api/recipes/${location?.id}/${id}`);
    revalidatePath("/recipes");
  } catch (error) {
    throw error;
  }
