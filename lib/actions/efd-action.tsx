...
import { SoldItemsReport } from "@/types/product/type";
...
export const EfdStatus = async () => {
  ...
  try {
    const status = await apiClient.get(`/api/vfd/${location?.id}/status`);
...
export const ZReports = async (
  ...
  try {
    ...
    // Send dates in the request body — the backend expects startDate/endDate
    ...
    const report = await apiClient.post(
      `/api/vfd/${location.id}/z-report`,
      body,
    );

    console.log("Z Report response:", report);
